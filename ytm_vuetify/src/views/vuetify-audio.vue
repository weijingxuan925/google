<template>
  <v-card style="text-align: center" :flat="flat == undefined || flat == false  ? false : true">
      <v-card-text>
          ...
          <v-btn outlined icon class="ma-2" :color="color" @click.enter="stop()" :disabled="!loaded">
              <v-icon>{{ stopIcon }}</v-icon>
          </v-btn>
          <v-btn outlined icon class="ma-2" :color="color" @click.enter="mute()" :disabled="!loaded">
              <v-icon v-if="!isMuted">{{ volumeHighIcon }}</v-icon>
              <v-icon v-else>{{ volumeMuteIcon }}</v-icon>
          </v-btn>
          <v-btn outlined icon class="ma-2" :color="color" @click.enter="loaded ? download() : reload()" v-if="!loaded">
              <v-icon>{{ refreshIcon }}</v-icon>
          </v-btn>
          <v-btn outlined icon class="ma-2" :color="color" @click.enter="loaded ? download() : reload()" v-if="loaded && downloadable">
              <v-icon>{{ downloadIcon }}</v-icon>
          </v-btn>
          <v-slider v-model="playerVolume" :prepend-icon="volumeHighIcon" max="1" step="0.01" min="0"></v-slider>
          <v-progress-linear v-model="percentage" height="5" style="margin-top: 15px; margin-bottom: 15px;" @click.enter="setPosition()" :disabled="!loaded"></v-progress-linear>
